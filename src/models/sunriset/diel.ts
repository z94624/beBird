...
import { GeoDataEnum } from '../enum/geoEnum';
...
export class SUNRISETGetSunriseSunsetTimesReq {
	constructor(data?: SUNRISETGetSunriseSunsetTimesReq) {
		if (!data) return;
		const {
			lat = GeoDataEnum.LATITUDE_OF_TAIWAN as string,
			lng = GeoDataEnum.LONGITUDE_OF_TAIWAN as string,
			date = undefined,
			timezone = undefined,
			date_start = undefined,
			date_end = undefined,
			time_format = TimeFormatEnum['24_HR'],
		} = data;
		this.lat = lat;
		this.lng = lng;
		this.date = date;
	...
	lat: string = GeoDataEnum.LATITUDE_OF_TAIWAN as string; // Latitude of the location in decimal degrees. Example: 38.907192
	lng: string = GeoDataEnum.LONGITUDE_OF_TAIWAN as string; // Longitude of the location in decimal degrees. Example: -77.036873
	...
	time_format?: TimeFormatEnum = TimeFormatEnum['24_HR']; // Format that all the times are displayed in. Default is a 12-hour clock but acceptable values include “24” for a 24-hour clock, “military” for military time, and “unix” for unix timestamps. Note that unix timestamps are only returned in UTC.
}
