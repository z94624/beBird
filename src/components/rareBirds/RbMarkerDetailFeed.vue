<template>
	<q-list dense separator>
		<template v-for="(media, mIdx) in mediaList" :key="mIdx">
			<q-item v-if="media.onClick" v-ripple clickable @click="media.onClick">
				<div class="w-full flex no-wrap items-center gap-4">
					<div class="bg-secondary w-[33.33px] h-[33.33px] m-[3.335px] flex justify-center items-center"
						style="border-radius: 50%">
						<q-icon :name="media.customIcon ? `img:${media.icon}` : media.icon" color="primary" size="xs" />
					</div>
			...
			<q-item v-else v-ripple clickable>
				<ShareNetwork :description="descTEXT" :hashtags="userComName" :network="media.network"
					:title="`${userComName} [${obs?.sciName ?? '{sciName}'}]`" :url="checklistUrl"
					class="w-full flex no-wrap items-center gap-4">
					<q-icon :class="[media.customIcon ? 'm-[3.335px]' : '']"
						:name="media.customIcon ? `img:${media.icon}` : media.icon"
						:size="media.customIcon ? '33.33px' : '40px'" :style="{
