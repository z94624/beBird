<template>
	<VlMap ref="mapRef">
		<template #search-menu>
			<q-form class="q-gutter-md" no-error-focus no-reset-focus>
				<CountrySelect :country="country" @update:country="onUpdateCountry" />
				<RegionSelect :country="country" :region="region" @loading="(status: boolean) => (loading = status)"
		...
		<template #markers>
			<l-marker v-for="(obs, oIdx) in pureObsList" :key="oIdx" :lat-lng="[obs.lat, obs.lng]"
				@click="onClickMarker">
				<VlIcon />
				...
				<VlPopup :navigation-url="getGoogleMapsPlaceURL(obs.lat, obs.lng)"
					...
					<template #content>
						<HrefButton :href="`https://ebird.org/species/${obs.speciesCode}`">
							<q-badge :label="`×${obs.howMany}`" color="warning" floating rounded />
